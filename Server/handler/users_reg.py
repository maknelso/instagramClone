...
class users_reg(Resource):
    ...
    def post(self):
        ...
        hashedPassword = bcrypt.generate_password_hash(data["password"]).decode("utf-8")
        user = Account(
            email=data["email"],
            name=data["name"],
            username=data["username"],
            password=hashedPassword,
            avatar="https://instagramclone-2022.s3.us-west-1.amazonaws.com/avatars/profile.jpg",
        )
        db.session.add(user)
        db.session.commit()
        users = Account.query.all()
